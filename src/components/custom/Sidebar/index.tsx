...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`absolute left-0 top-0 z-10 flex h-screen w-60 flex-col overflow-y-hidden bg-[#fafafa] duration-300 ease-linear lg:static lg:translate-x-0 ${
                  ...
                  >
                    {(handleClick, open) => (
                      <React.Fragment>
                        <NavLink
                          to={path}
                          className={`group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium text-black duration-300 ease-in-out hover:bg-[#dbdddd] dark:hover:bg-meta-4 ${
                            pathname === path || pathname.includes(path)
                              ? 'bg-[#f1f4f4] text-black'
                              : ''
                          }`}
                          onClick={(e) => {
                            sidebarExpanded
